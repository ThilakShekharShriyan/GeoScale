...
def main():
    ...
    if world_size > 1:
        if use_gpu:
            ...
            # ViT FSDP with bf16 Mixed Precision
            ...
            # Fallback to standard DDP on CPU
            model = DDP(model)
        
    dataset = ShardedGeoDataset(args.data_dir)
    sampler = DistributedSampler(dataset) if world_size > 1 else None
    
    # Configure number of workers for dataloader
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            num_batches += 1
            images_in_batch = images.size(0) * world_size # approximate total processed
